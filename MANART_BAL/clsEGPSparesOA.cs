...
{
    /// <summary>
    /// Summary description for clsEGPSparesOA
    /// </summary>
    public class clsEGPSparesOA
    {
        String sPONo = "";
        public clsEGPSparesOA()
        {
            //
        ...
        private bool bSaveHeader(clsDB objDB, string sDealerCode, DataTable dtHdr, DataTable dtOATaxDetails, ref int iHdrID)
        {
            ...
            try
            {
                if (Func.Convert.iConvertToInt(dtHdr.Rows[0]["ID"]) == 0)
                {
                    ...
                    sPONo = Func.Convert.sConvertToString(dtHdr.Rows[0]["oa_no"]);
                    iHdrID = objDB.ExecuteStoredProcedure("SP_SparesOAHdr_Save", dtHdr.Rows[0]["ID"], dtHdr.Rows[0]["oa_no"], dtHdr.Rows[0]["oa_Date"], dtHdr.Rows[0]["CustID"], dtHdr.Rows[0]["Dealer_ID"], dtHdr.Rows[0]["oa_close"], dtHdr.Rows[0]["oa_canc"], dtHdr.Rows[0]["oa_com"], dtHdr.Rows[0]["oa_lock"], dtOATaxDetails.Rows[0]["net_tr_amt"], dtHdr.Rows[0]["reference"], dtHdr.Rows[0]["narration"], dtHdr.Rows[0]["oa_validity_date"], dtOATaxDetails.Rows[0]["discount_amt"], dtOATaxDetails.Rows[0]["before_tax_amt"], dtOATaxDetails.Rows[0]["mst_amt"], dtOATaxDetails.Rows[0]["cst_amt"], dtOATaxDetails.Rows[0]["surcharge_amt"], dtOATaxDetails.Rows[0]["tot_amt"], dtOATaxDetails.Rows[0]["pf_per"], dtOATaxDetails.Rows[0]["pf_amt"], dtOATaxDetails.Rows[0]["other_per"], dtOATaxDetails.Rows[0]["other_money"], dtOATaxDetails.Rows[0]["oa_tot"]);
            ...
            try
            {
                ...
                for (int iRowCnt = 0; iRowCnt < dtDet.Rows.Count; iRowCnt++)
                {
                    if (dtDet.Rows[iRowCnt]["Status"] != "D")
                    {
                        ...
                        {
                            objDB.ExecuteStoredProcedure("SP_OADet_Save", dtDet.Rows[iRowCnt]["ID"], iHdrID, dtDet.Rows[iRowCnt]["Part_ID"], dtDet.Rows[iRowCnt]["Qty"], dtDet.Rows[iRowCnt]["MRPRate"], dtDet.Rows[iRowCnt]["bal_qty"], dtDet.Rows[iRowCnt]["discount_per"], dtDet.Rows[iRowCnt]["discount_amt"], dtDet.Rows[iRowCnt]["disc_rate"], dtDet.Rows[iRowCnt]["Total"], dtDet.Rows[iRowCnt]["PartTaxID"]);
                        }
                    }
        ...
        public DataSet GetOA(int OAId, string OAType, int DealerID, int OATypeID) //change for REMAN PO
        {
            ...
            {
                DataSet ds;
                ds = objDB.ExecuteStoredProcedureAndGetDataset("SP_Get_OA", OAId, OAType, DealerID);
                return ds;
            }
            catch
            {
        ...
        public DataSet UploadPartDetailsAndGetPartDetails(string sFileName, int iDealerId, DataTable dtDet, int iCustID)
        {
            DataSet ds = null;
            ...
            try
            {
                ...
                {
                    ds = objDB.ExecuteStoredProcedureAndGetDataset("SP_EGP_ImportSparesOAPartDetailsFromExcelSheet", sFileName, iDealerId, iCustID);
                }
        ...
        private bool bSaveUploadedPartDetails(string sFileName, int iDealerId, DataTable dtDet)
        {
            ...
            try
            {
                ...
                for (int iRowCnt = 0; iRowCnt < dtDet.Rows.Count; iRowCnt++)
                {
                    objDB.ExecuteStoredProcedure("SP_EGP_InsertSparesOAPartDetailsFromExcelSheet", sFileName, dtDet.Rows[iRowCnt]["PartNo"], dtDet.Rows[iRowCnt]["Quantity"], bNewTable);
                    bNewTable = "N";
                }
                objDB.CommitTransaction();
                bSaveRecord = true;
