...
{
    public partial class frmStockNormsCategorywise : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ToolbarC.iValidationIdForSave = 73;
                ...
                if (iMenuId == 706 && sUserRole == "3")// ASM User
                {
                    Location1.Visible = false;
                    Table2.Visible = false;
                    divBtnShow.Style.Add("display", "");
                    ToolbarC.Visible = false;
                    tblFMSCatUpload.Visible = false;
                    Location.Visible = true;
                }
                    else if (iMenuId == 706 && (sLoginName == "DCAN703161" || sLoginName == "DCAN703114"))
                //else if (iMenuId == 706 && sUserRole == "1")// Head User
                {
                    Location1.Visible = false;
                    Table2.Visible = false;
                    divBtnShow.Style.Add("display", "");
                ...
                {// Dealer User
                    ...
                    Location1.Visible = false;//Dealer Details
        ...
        private void DisplayCurrentRecord()
        {
            try
            {
                ...
                ds = objStkNorms.GetDealerStockingNormsCatWise(iID, sDealerIds, "All");
                if (ds != null)
                {
                    GrdStockingNormsCat.DataSource = ds.Tables[0];
                    GrdStockingNormsCat.DataBind();
            ...
            try
            {
                if ((txtFilePath.HasFile))
                {
                    ...
                    if (strFileExtType == ".xls" || strFileExtType == ".xlsx")
                        txtFilePath.SaveAs(Server.MapPath("~/DownLoadFiles/StockingNorms/" + strFileName + strFileType));
                    else
                    {
                    ...
                    string strNewPath = Server.MapPath("~/DownLoadFiles/StockingNorms/" + strFileName + strFileType);
                    ...
                    try
                    {
                        ...
                        dsUploadPartsTarget = objStkNorms.UploadPartsStockingNormsDetails(txtFilePath.FileName, dtHdr, iUserId);
                        ...
                        if (objStkNorms.bStockingNorms(dtHdr, iUserId) == true)
                        {

                            int TotalRecords = 0;
                            int TotalSuccess = 0;
                            int TotalError = 0;
                            ViewState["StkNorms"] = null;
                            ViewState["StkNorms"] = ds.Tables[0];
